...
class ResNet(nn.Module):
    ...
    def __init__(self, block=BasicBlock, keep_prob=1.0, avg_pool=True, drop_rate=0.1, dropblock_size=5, branch=False):
        self.inplanes = 3
        ...
        self.layer4 = self._make_layer(block, 640, stride=2, drop_rate=drop_rate, drop_block=True,
                                       block_size=dropblock_size)
        if self.branch:
            self.layer4_1 = self._make_layer(block, 640, stride=2, drop_rate=drop_rate, drop_block=True,
                                             block_size=dropblock_size)
    ...
    def forward(self, x):
        ...
        if self.branch:
            mu = self.max_pool(x3)
            sig = self.max_pool(self.layer4_1(x3))
            return [mu, sig]
        return x4
